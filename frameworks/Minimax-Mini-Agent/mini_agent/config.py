    ...
    def from_yaml(cls, config_path: str | Path) -> "Config":
        ...
        if secrets_file.exists():
            try:
                with open(secrets_file, encoding="utf-8") as f:
                    for line in f:
                        if line.startswith('MiniMax_M2_CODE_PLAN_API_KEY='):
                            # Extract the value after the equals sign, handling quotes
                ...
                if secrets_api_key:
                    print(f"🔐 Using API key from secrets file: {secrets_file}")
            except Exception as e:
                print(f"⚠️  Warning: Could not read secrets file {secrets_file}: {e}")

        # Parse LLM configuration
        if "api_key" not in data:
            raise ValueError("Configuration file missing required field: api_key")

        # Use secrets API key if available, otherwise use config file
        api_key = secrets_api_key if secrets_api_key else data["api_key"]

        if not api_key or api_key == "YOUR_API_KEY_HERE":
            raise ValueError("Please configure a valid API Key in /adapt/secrets/m2.env or config.yaml")
