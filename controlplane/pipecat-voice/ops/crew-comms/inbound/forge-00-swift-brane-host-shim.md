...
- `/adapt/platform/novaops/controlplane/pipecat-voice/swift-brane/Cargo.toml` + `crates/file-read/` scaffold
- `/adapt/platform/novaops/controlplane/pipecat-voice/swift-brane/crates/file-read/src/lib.rs` — no_std wasm64 file_read stub (returns 42)
- `/adapt/platform/novaops/controlplane/pipecat-voice/swift-brane/README.md` — verified build command
...
3. **Write** host shim at `/adapt/platform/novaops/controlplane/pipecat-voice/swift-brane/host/shim.rs`:
   - Use `wasmtime` or raw FFI to load the wasm64 module
   - Call `file_read(path_ptr, path_len)` and return result
4. **Write** Swift Brane registry at `/adapt/platform/novaops/controlplane/pipecat-voice/scripts/swift_registry.json`:
   ```json
